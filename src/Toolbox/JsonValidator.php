...
class JsonValidator {
  ...
  public function validate_json( $json, &$validated_params ) : array {
    $errors = [];
    ...
    foreach ( $this->parameters as $parameter ) {
      ...
      switch ( $parameter['type'] ) {
        case 'number':
          ...
          break;
        case 'text':
          if ( !is_string( $value ) || strlen( $value ) > $parameter['length'] ) {
            $message = "'" . $parameter['key'] . "' must be valid text with length less than or equal to " . $parameter['length'] . ".";
            break;
          }
